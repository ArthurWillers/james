                    ...
                    <x-table.cell align="right">
                        @if($settlement->settlement_group_id === null)
                            <div class="flex justify-end gap-2 w-full">
                                <x-button type="button" color="outline" class="bg-white hover:bg-neutral-50 text-neutral-600 border-neutral-300" @click="openRestore({{ $settlement->id }}, '{{ addslashes($settlement->description) }}')">
                                    <x-heroicon-o-arrow-uturn-left class="size-4" />
                                    Restaurar
                                </x-button>

                                <x-button type="button" color="outline" class="bg-white hover:bg-red-50 text-red-600 border-red-200" @click="openForceDelete({{ $settlement->id }}, '{{ addslashes($settlement->description) }}')">
