...
<label class="flex-1 h-full">
    ...
    <div class="flex flex-col h-full items-center justify-center py-3 rounded-lg transition-all cursor-pointer peer-checked:bg-white peer-checked:shadow-sm {{ $activeClass }} {{ $inactiveClass }} {{ $attributes->get('class') }}">
        @if($icon)
