...
<div {{ $attributes->except(['x-model', 'class', 'title', 'style', 'x-bind:style', ':style']) }} class="relative w-10 h-10 rounded-full overflow-hidden shrink-0 border border-neutral-300 shadow-sm transition-transform hover:scale-110 focus-within:ring-2 focus-within:ring-accent focus-within:ring-offset-2 focus-within:scale-110 flex items-center justify-center bg-white cursor-pointer group" title="{{ $attributes->get('title', 'Escolher cor personalizada') }}">
