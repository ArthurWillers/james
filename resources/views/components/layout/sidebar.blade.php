<div x-data="{ open: false }">
    <aside
        ...
        <div class="flex items-center">
            ...
            <button @click="open = !open"
                ...
                <x-icons.heroicons.outline.x-mark class="w-6 h-6" />
            </button>
        </div>
        ...
        <x-ui.dropdown position="top" class="mt-auto hidden lg:block" accent contentClass="w-full">
            <x-slot name="trigger">
                <button class="w-full flex items-center rounded-lg p-1 hover:bg-neutral-800/5 group cursor-pointer">
                    ...
                    <div class="ms-auto text-neutral-800/80 group-hover:text-neutral-800">
                        <x-icons.heroicons.outline.chevron-up-down class="w-6 h-6" />
                    </div>
                </button>
            </x-slot>
            ...
            <x-slot name="content">
                ...
                <a href="{{ route('settings') }}" @click="open = !open"
                    ...
                    <x-icons.heroicons.outline.cog-6-tooth class="w-5 h-5" />
                    Configurações
                </a>
                <form method="POST" id="logout" action="{{ route('logout') }}">
                    ...
                    <button type="submit" @click="open = !open"
                        ...
                        <x-icons.heroicons.mini.arrow-right-start-on-rectangle class="w-5 h-5" />
                        Sair
                    </button>
                </form>
    ...
    <header class="flex items-center px-6 w-full min-h-14 lg:hidden">
        <button @click="open = !open" class="cursor-pointer rounded-lg p-1 hover:bg-neutral-200">
            <x-icons.heroicons.outline.bars-3 class="w-6 h-6" />
        </button>
        ...
        <x-ui.dropdown position="bottom-end" class="ms-auto" accent contentClass="w-60">
            <x-slot name="trigger">
                <button
                    ...
                    <div class="ms-auto text-neutral-800/80 group-hover:text-neutral-800">
                        <x-icons.heroicons.mini.chevron-down class="w-4 h-4" />
                    </div>
                </button>
            </x-slot>
